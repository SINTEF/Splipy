...
class TestSurface(unittest.TestCase):
    def test_constructor(self):
        ...
        with self.assertRaises(TypeError):
            surf = Surface(2,2, [0,0,1,1], [0,0,1,1], controlpoints, True, 1)  # too many arguments
        with self.assertRaises(TypeError):
            surf = Surface("tree-fiddy",2,[0,0,1,1], [0,0,1,1], controlpoints) # wrong argument type
        with self.assertRaises(Exception):
            surf = Surface(2,2,[1,1,0,0], [0,0,1,1], controlpoints)            # illegal knot vector
        with self.assertRaises(Exception):
            surf = Surface(2,2,[0,0,0,1], [0,0,1,1], controlpoints)            # illegal knot vector
    ...
    def test_force_rational(self):
        ...
        surf2             = surf.ForceRational()
        evaluation_point2 = surf2.Evaluate(0.23, .66)
        control_point2    = surf2[0]
