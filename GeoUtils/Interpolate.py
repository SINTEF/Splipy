...
def Linear(x,y=[],z=[]):
	...
	return GoTools.CurveFactory.InterpolateCurve(pts, t, 2)
...
def Cubic(x, y=[], z=[]):
	...
	for i in range(N):
		...
		if(i>0):
			t.append(t[i-1] + abs(pts[i]-pts[i-1])) # eucledian distance between points i and (i-1)
	return GoTools.CurveFactory.InterpolateCurve(pts, t, 4)
...
def UniformCubic(x, y=[], z=[]):
	...
	for i in range(N):
		pts.append(GoTools.Point(x[i],y[i],z[i]))
	return GoTools.CurveFactory.InterpolateCurve(pts, range(N), 4)
...
def UniformCubicP(pts):
	...
	@rtype: Curve
	"""
	return GoTools.CurveFactory.InterpolateCurve(pts, range(len(pts)), 4)
