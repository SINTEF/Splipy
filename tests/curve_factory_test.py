...
class TestCurveFactory(unittest.TestCase):
    ...
    def test_circle_segment_from_three_points(self):
        ...
        c = cf.circle_segment_from_three_points(
            [1.0 / sqrt(2), 1.0 / sqrt(2), 0], [0.5, 0.5, 1 / sqrt(2)], [0, 0, 1]
        )
        t = np.linspace(c.start(0), c.end(0), 25)
        x = c.evaluate(t)
        self.assertEqual(c.dimension, 3)
        ...
        c = cf.circle_segment_from_three_points(
            [0.5, 0.5, 1 / sqrt(2)], [2 ** (-3.0 / 2), 2 ** (-3.0 / 2), sqrt(3) / 2], [0, 0, 1]
        )
        t = np.linspace(c.start(0), c.end(0), 25)
        x = c.evaluate(t)
        for pt in np.array(x):
        ...
        c = cf.circle_segment_from_three_points(
            [1.5, 0.5, 1 / sqrt(2)], [1 + 2 ** (-3.0 / 2), 2 ** (-3.0 / 2), sqrt(3) / 2], [1, 0, 1]
        )
        t = np.linspace(c.start(0), c.end(0), 25)
        x = c.evaluate(t)
        for pt in np.array(x):
        ...
        self.assertTrue(np.allclose(c.binormal(0.5), [3.0 / 5, -4.0 / 5, 0]))
    ...
    def test_ellipse(self):
        ...
        c = cf.ellipse(1, 5, center=(0.3, 6))
        t = np.linspace(c.start(0), c.end(0), 25)
        for pt in c.evaluate(t):
            x, y = pt
            self.assertAlmostEqual(((x - 0.3) / 1) ** 2 + ((y - 6) / 5) ** 2, 1)
