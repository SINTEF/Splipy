...
class TestSurfaceFactory(unittest.TestCase):
    ...
    def test_curve_interpolation(self):
        basis = BSplineBasis(4, [0, 0, 0, 0, 0.3, 0.9, 1, 1, 1, 1])
        ...
        self.assertAlmostEqual(crv(0.4)[0], 0.4**2)  # x=t^2
        self.assertAlmostEqual(crv(0.4)[1], 1 - 0.4)  # y=1-t
        self.assertAlmostEqual(crv(0.4)[2], 0.4**3 + 2 * 0.4)  # z=t^3+2t
        self.assertAlmostEqual(crv(0.5)[0], 0.5**2)  # x=t^2
        self.assertAlmostEqual(crv(0.5)[1], 1 - 0.5)  # y=1-t
        self.assertAlmostEqual(crv(0.5)[2], 0.5**3 + 2 * 0.5)  # z=t^3+2t
    ...
    def test_surface_torus(self):
        ...
        for pt in np.array(x[0, :, :]):
            self.assertAlmostEqual(
                pt[0] * pt[0] + pt[1] * pt[1], (3 - 1.0 / sqrt(2)) ** 2
            )  # check radius=3-1/sqrt(2)
    ...
    def test_thicken(self):
        ...
        s = sf.thicken(c, 0.5)  # extend to y=[-.5, .5]
        ...
        self.assertTupleEqual(s.bounding_box()[1], (-0.5, 0.5))
    ...
    def test_interpolate(self):
        ...
        x[:, :, 1] = U - V
        x[:, :, 2] = U * U * V * V * V - U * V + 3
        b1 = BSplineBasis(3, [0, 0, 0, 0.33, 0.66, 0.7, 0.8, 1, 1, 1])
        b2 = BSplineBasis(4, [0, 0, 0, 0, 0.1, 0.2, 0.5, 1, 1, 1, 1])
