...
class TestVolumeFactory(unittest.TestCase):
    def test_cylinder_volume(self):
        ...
        for u in np.linspace(vol.start()[0], vol.end()[0], 5):
            for v in np.linspace(vol.start()[1], vol.end()[1], 5):
                for w in np.linspace(vol.start()[2], vol.end()[2], 5):
                    x = vol(u, v, w)
                    self.assertAlmostEqual(np.linalg.norm(x[:2], 2), u)  # (x,y) coordinates to z-axis
        ...
        vol = vf.cylinder(r=2, h=5, center=(0, 0, 1), axis=(1, 0, 0))
        for u in np.linspace(vol.start()[0], vol.end()[0], 5):
            for v in np.linspace(vol.start()[1], vol.end()[1], 5):
                for w in np.linspace(vol.start()[2], vol.end()[2], 5):
                    x = vol(u, v, w)
        ...
        # test xaxis
        vol = vf.cylinder(r=sqrt(2), xaxis=(1, 1, 0))
        u = vol.end("u")
    ...
    def test_edge_surfaces(self):
        ...
        basis2 = BSplineBasis(3, [0, 0, 0, 0.64, 2, 2, 2])
        ...
        basis1 = BSplineBasis(3, [0, 0, 0, 1, 1, 1])
        basis2 = BSplineBasis(2, [0, 0, 0.4, 0.44, 1, 1])
        ...
        for u in np.linspace(0, 1, 7):
            for v in np.linspace(0, 1, 7):
                for w in np.linspace(0, 1, 2):  # rational basis, not linear in w-direction
                    self.assertAlmostEqual(
                        vol(u, v, w)[0], bottom(u, v)[0] * (1 - w) + top(u, v)[0] * w
                    )  # x-coordinate
                    self.assertAlmostEqual(
                        vol(u, v, w)[1], bottom(u, v)[1] * (1 - w) + top(u, v)[1] * w
                    )  # y-coordinate
    ...
    def test_volume_loft(self):
        ...
        for c in [crv1, crv2, crv3, crv4]:
            c2 = c.clone()
            c2.project("z")
            surf.append(sf.edge_curves(c, c2))
    ...
    def test_revolve(self):
        ...
        vol = vf.revolve(square, theta=pi / 3)
        vol.reparam()  # set parametric space to (0,1)^3
        u = np.linspace(0, 1, 7)
        v = np.linspace(0, 1, 7)
        w = np.linspace(0, 1, 7)
        ...
        # test axis revolve
        vol = vf.revolve(Surface() + (1, 1), theta=pi / 3, axis=(1, 0, 0))
        vol.reparam()  # set parametric space to (0,1)^3
        u = np.linspace(0, 1, 7)
        v = np.linspace(0, 1, 7)
        w = np.linspace(0, 1, 7)
    ...
    def test_interpolate(self):
        ...
        x[:, :, :, 1] = U * U * W
        x[:, :, :, 2] = W * W * W + U * V
        b1 = BSplineBasis(3, [0, 0, 0, 0.33, 0.66, 1, 1, 1])
        b2 = BSplineBasis(4, [0, 0, 0, 0, 0.5, 1, 1, 1, 1])
        b3 = BSplineBasis(4, [0, 0, 0, 0, 0.2, 1, 1, 1, 1])
    ...
    def test_l2(self):
        ...
        vol = vf.least_square_fit(x, [b1, b2, b3], [t, t, t])
        t = np.linspace(0, 1, 7)
        V, U, W = np.meshgrid(t, t, t)
        x = vol(t, t, t)
