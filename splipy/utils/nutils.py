...
def multiplicities(spline: SplineObject) -> list[list[int]]:
    ...
    """
    return [
        [spline.order(d) - spline.bases[d].continuity(k) - 1 for k in spline.knots(d)]
        for d in range(spline.pardim)
    ]
...
def splipy_to_nutils(spline: SplineObject) -> tuple[mesh.Domain, function.Function]:
    ...
    # TODO(Kjetil): add correct behaviour for rational and/or periodic geometries
    return domain, geom
