...
class GRDECL(MasterIO):
    ...
    def read(self):
        for line in self.fstream:
            ...
            if line == "specgrid":
                ...
                self.attribute[line] = np.array(self.cell_property(), dtype=dtype)
    ...
    def write(self, obj: Union[SplineObject, SplineModel, Sequence[SplineObject]]) -> None:
        raise OSError("Writing to GRDECL not supported")
    ...
    def texture(self, p, ngeom, ntexture, method="full", irange=[None, None], jrange=[None, None]):
        ...
        for name in self.attribute:
            ...
            # TODO(Kjetil): This flattens the image if it happens to be 3D (or higher...)
            # do we need a way to communicate the structure back to the caller?
            # data = data.reshape(-1, data.shape[-1])

            # TODO(Kjetil): This normalizes the image,
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        self.fstream.__exit__(exc_type, exc_value, traceback)
