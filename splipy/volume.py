...
from typing import Optional, Tuple, List, Sequence, Union, cast
...
class Volume(SplineObject):
    ...
    def __init__(
        ...
        """
        super().__init__([basis1, basis2, basis3], controlpoints, rational, raw)
    ...
    def edges(self) -> Tuple[Curve, ...]:
        ...
        :rtype: (Curve)
        """
        return tuple(cast(Curve, self.section(*args)) for args in sections(3, 1))

    def faces(self) -> Tuple[Optional[Surface], ...]:
        """Return the six faces of this volume in order: umin, umax, vmin, vmax, wmin, wmax.

        :return: Boundary faces
        :rtype: (Surface)
        """
        boundary_faces: List[Optional[Surface]] = [cast(Surface, self.section(*args)) for args in sections(3, 2)]
    ...
    def rebuild(self, p: Union[int, Sequence[int]], n: Union[int, Sequence[int]]) -> Volume:
        ...
        :rtype: Volume
        """
        order = ensure_listlike(p, dups=3)
        ncps = ensure_listlike(n, dups=3)
        ...
        for i in range(3):
            knot = [0] * order[i] + list(range(1, ncps[i] - order[i] + 1)) + [ncps[i] - order[i] + 1] * order[i]
            basis.append(BSplineBasis(order[i], knot))
        ...
        cp = cp.reshape(ncps[0] * ncps[1] * ncps[2], cp.shape[3])
